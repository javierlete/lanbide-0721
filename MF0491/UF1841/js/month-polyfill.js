...
(function () {
    ...
    (function ($) {
        $.fn.inputMonth = function () {
            var advanceMonth, compareMonths, decrement, increment, makeMonthDisplayString, makeMonthString, readMonth, regressMonth, stepNormalize;
            readMonth = function (m_str) {
                var matchData, monthPart, yearPart;
            ...
            makeMonthString = function (month_obj) {
                var m_arr;
                m_arr = [month_obj['year'].toString()];
                m_arr.push('-');
                if (month_obj['month'] < 10) {
                    m_arr.push('0');
                }
                m_arr.push(month_obj['month'].toString());
                return m_arr.join('');
            };
            makeMonthDisplayString = function (month_obj, elem) {
                var $elem, month_arr, month_names;
                $elem = $(elem);
            ...
            advanceMonth = function (inMonth, amt) {
                var outMonth;
                outMonth = {
                    year: inMonth['year'],
                    month: inMonth['month']
            ...
            regressMonth = function (inMonth, amt) {
                var outMonth;
                outMonth = {
                    year: inMonth['year'],
                    month: inMonth['month']
            ...
            increment = function (hiddenField, monthBtn, calendarDiv) {
                ...
                if ((max != null) && compareMonths(value, max) === -1) {
                    value['year'] = max['year'];
            ...
            decrement = function (hiddenField, monthBtn, calendarDiv) {
                ...
                if ((min != null) && compareMonths(value, min) === 1) {
                    value['year'] = min['year'];
            ...
            stepNormalize = function (inMonth, hiddenField) {
                var $hiddenField, kNum, max, min, minNum, step, stepDiff, stepDiff2;
                $hiddenField = $(hiddenField);
                step = $hiddenField.data("step");
                min = $hiddenField.data("min");
                max = $hiddenField.data("max");
            ...
            $(this).filter('input[type="month"]').each(function () {
                var $calendarContainer, $calendarDiv, $hiddenField, $monthBtn, $this, calendarContainer, calendarDiv, className, closeFunc, hiddenField, max, min, monthBtn, step, style, value;
                $this = $(this);
                ...
                if ((value != null) && /^\d{4,}-\d\d$/.test(value)) {
                ...
                $monthBtn.text(makeMonthDisplayString(value, calendarDiv));
                if (min != null) {
                    $calendarDiv.datepicker("option", "minDate", new Date(min['year'], min['month'] - 1, 1));
                }
                if (max != null) {
                    $calendarDiv.datepicker("option", "maxDate", new Date(max['year'], max['month'], 0));
                }
